...
export const TaskInfo = observer(() => {
    ...
    return <div className={styles.TaskInfo}>info :)
        ...
        {taskStore.currentTask && <div>


            <div style={{'margin': '10px'}}>
                <TextField value={title} onChange={(e: any) => setTitle(e.currentTarget.value)} size={"small"}/>


                <Button variant={"contained"}
                        onClick={setTitleHandler}>add subtask</Button>

            </div>
            <div style={{'margin': '10px'}}>
                <TextField value={description} onChange={(e: any) => setDescription(e.currentTarget.value)}
                           size={"small"}/>
                <Button variant={"contained"}
                        onClick={setEditTaskDescriptionHandler}>edit
                    description</Button>
            </div>
        ...
        <div>find task</div>
        <TextField onFocus={()=>setFindRes('')} value={find} onChange={(e: any) => setFind(e.currentTarget.value)} size={"small"}/>
